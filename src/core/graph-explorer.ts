...
export class GraphExplorer {
      ...
      while (nodesToExplore.length > 0 && resultNodes.length < maxNodes) {
        ...
        for (const edge of edges) {
          ...
          if (exploredNodes.has(connectedNodeId)) {
            ...
            if (includeEdges && !resultEdges.some(e => 
          ...
          const connectedNode = await nodeLookup.getNode(graphId, connectedNodeId)
          if (!connectedNode) {
          ...
          // Skip excluded types
          if (options.excludeTypes && options.excludeTypes.includes(connectedNode.type)) {
            continue
          }
  ...
  public async searchNodes(
    ...
    try {
      logger.info(`Searching for nodes in graph ${graphId} with query "${query}"`)
      ...
      let matchingNodes = graph.nodes.filter(node => {
        ...
        if (exactMatch) {
          return (
            ...
            (node.description && node.description.toLowerCase() === normalizedQuery)
          )
        } else {
          return (
      ...
      if (includeEdges && resultNodes.length > 0) {
        ...
        resultEdges = graph.edges.filter(edge => 
  ...
  public async findPath(
    ...
    try {
      logger.info(`Finding path from ${fromNodeQuery} to ${toNodeQuery} in graph ${graphId}`)
      ...
      if (!fromNode || !toNode) {
        logger.warn(`One or both nodes not found: ${fromNodeQuery}, ${toNodeQuery}`)
        return {
          path: [],
          edges: [],
          length: 0,
      ...
      while (queue.length > 0) {
        ...
        // Filter by relation types if specified
        const filteredEdges = options.relationTypes && options.relationTypes.length > 0
          ? outgoingEdges.filter(edge => options.relationTypes!.includes(edge.type))
          : outgoingEdges
   ...
   */
  public async getGraphStatistics(
    graphId: string
  ): Promise<{
    totalNodes: number
    totalEdges: number
    ...
    try {
      ...
      const totalConnections = Object.values(connections).reduce((sum, count) => sum + count, 0)
      const avgConnections = graph.nodes.length > 0
